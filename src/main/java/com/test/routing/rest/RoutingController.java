...
public class RoutingController
{
    ...
    {
        final var countries = routingService.search( origin, destination );
        return countries.map( c -> Route.builder().route( c ).build() ).orElseThrow(
    ...
    {
        return ResponseEntity.status( HttpStatus.BAD_REQUEST ).body( Error.builder().error( ex.getMessage() ).build() );
    }
}
